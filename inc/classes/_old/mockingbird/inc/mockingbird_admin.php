...
class Mockingbird_admin extends Mockingbird {
	...
	public function custom_meta_render($object, $box){
		...
		//Set Posttype Filter
		$this->options->types_to_display = count($this->options->types_to_display) > 0 ? $this->options->types_to_display : get_post_types( array('public' => true) );
		unset($this->options->types_to_display['attachment']);
	...
	?>
		...
		<?php if($posts) : ?>
			...
			<div class="sc-mockingbird-filter-select">
				...
				<div>
					<!-- Loading Indicator -->
					<!-- <img class="sc-loading-image" src="<?php echo MOCKINGBIRD_URL; ?>/images/icon_loading.gif" /> -->
					<!-- <img class="sc-loading-image" src="<?php echo MOCKINGBIRD_URL; ?>/images/icon_spinner.gif" /> -->
				...
				<?php foreach( $posts as $post ): ?>
					...
					<li class="button">
						<span class="sc-mockingbird-label">
							<?php echo $this->options->context == 'side' ? esc_html__( Str::limit( $titles[$count] ? $titles[$count] : $post->post_title, 27 ) ) : esc_html__( $titles[$count] ? $titles[$count] : $post->post_title ) ; ?>
						</span>
    ...
    public function load_assets(){
    	...
    	if( in_array($typenow, $this->options->display_on) && $pagenow == "post.php" || $pagenow == "post-new.php" ){
    		//Load Javascript
        	wp_register_script( 'sc_mockingbird_script', MOCKINGBIRD_URL.'/js/admin_scirpt.js', false, null, true );
        	...
        	if( (float)$wp_version > 3.7 ){
				wp_enqueue_style( 'sc_mockingbird_admin_style', MOCKINGBIRD_URL.'/css/admin_style.css' );
			} else {
				wp_enqueue_style( 'sc_mockingbird_admin_style', MOCKINGBIRD_URL.'/css/admin_style_old.css' );
			}

...
class Mockingbird {
	...
	public function query_posts(){
		...
		if( $this->related_posts() ){
			//Get Posts
	        $args = array(
	            'post_type' 	 => $this->unique_key == 'sc_mockingbird' ? $this->options->types_to_display : $this->get_post_types(),
	            'order' 		 => 'ASC',
	            'orderby' 		 => 'post__in',
	            'posts_per_page' => -1,
	...
	public function render_list(){
        ...
        if( $post_query->posts ){
        	...
        	foreach( $post_query->posts as $post ){
        		...
        		$string .= "<a href='".get_permalink( $post->ID )."' title='".esc_html__( $post->custom_title )."'>";
    			...
    			if( $this->options->include_excerpt && ($content = $post->post_excerpt ? $post->post_excerpt : $post->post_content) ){
					...
					$content = preg_replace( '/\[mockingbird?[^\]]+?\]/', '', $content );
    		   		$string .= "<p>";
    		   		$string .= Sc_Str::limit( $content, $length );
    		   		$string .= $this->options->excerpt_link_text ? "<a class='readmore' href='".get_permalink( $post->ID )."' title='{$this->options->excerpt_link_text} &raquo;'>{$this->options->excerpt_link_text} &raquo;</a>" : "" ;
    		   		$string .= "</p>";
    			}
	...
	public function __construct( $post_id = false, $custom_styles = array(), $unique_key = '' ){
		...
		//Load CSS
        wp_enqueue_style( 'sc_mockingbird_style', MOCKINGBIRD_URL.'/css/mockingbird.css' );
...
function sc_mockingbird_post_filter( $content ){
	...
	if( isset( $setup->render ) && !has_filter( 'sc_mockingbird_post_filter' ) && ( is_single() || is_page() ) ){
